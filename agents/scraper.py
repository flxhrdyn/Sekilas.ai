...
class NewsScraper:
    ...
    def scrape_new_articles(self, max_per_source: int = 25) -> tuple[list[RawArticle], set[str]]:
        ...
        with httpx.Client(
            ...
            for source in sources:
                entries = self._load_rss_entries(client, source.url)
                count = 0
                for entry in entries:
                    ...
                    if not url or url in newly_processed:
                        continue

                    content = self._fetch_article_content(client, url)
                    if not content:
                        continue
    ...
    def _load_rss_entries(self, client: httpx.Client, feed_url: str) -> list[dict]:
        try:
