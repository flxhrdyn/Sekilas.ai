...
const SourceBadge = ({ source }: { source: string }) => {
  ...
  else if (s.includes('REUTERS')) { bg = 'bg-orange-600/20'; text = 'text-orange-400'; }
  else if (s.includes('ANTARA')) { bg = 'bg-sky-600/20'; text = 'text-sky-400'; }
  else if (s.includes('TECH IN ASIA')) { bg = 'bg-emerald-600/20'; text = 'text-emerald-400'; }
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12 space-y-16 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <section className="relative group">
        ...
        <div className="bg-brand-background/40 backdrop-blur-sm border border-brand-border/50 rounded-3xl p-10 md:p-14 text-center relative overflow-hidden">
          {/* Badge top center */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1.5 bg-brand-background border border-brand-border rounded-full flex items-center gap-2 shadow-2xl">
            <div className="w-1.5 h-1.5 rounded-full bg-brand-accent animate-pulse" />
            <span className="text-[10px] font-bold text-white/60 tracking-[0.2em] uppercase">Global Headline</span>
          </div>
          
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-extrabold text-white leading-[1.2] tracking-tight max-w-4xl mx-auto">
      ...
      <section className="space-y-8">
        ...
        <div className="space-y-12">
          {data.top_stories.map((story) => {
            const impact = deriveImpact(story);
            const reportsCount = story.articles.length;

            return (
              <div key={story.id} className="bg-brand-card/30 border border-brand-border/40 rounded-3xl overflow-hidden hover:border-brand-accent/30 transition-all duration-500">
                <div className="p-8 md:p-10 space-y-8">
                  {/* Header: Badges & Brain Icon */}
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                      <div className={`px-2.5 py-1 border rounded text-[10px] font-black uppercase tracking-widest ${getImpactColor(impact)}`}>
                        IMPACT: {impact}
                      </div>
                  ...
                  {/* Analyst Brief */}
                  <div className="bg-brand-background/50 border border-brand-border/40 rounded-2xl p-6 md:p-8 space-y-6">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Related Reports</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {story.articles.slice(0, 4).map((article, idx) => (
                        <a 
                          ...
                          className="flex items-center gap-3 group/article"
                        >
                          <SourceBadge source={article.source} />
                          <span className="text-xs text-brand-text-dim group-hover/article:text-brand-accent transition-colors truncate">
                            {article.title}
                          </span>
                        </a>
                      ))}
